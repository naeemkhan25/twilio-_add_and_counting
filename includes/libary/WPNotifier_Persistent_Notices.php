...
/**
 * Class Name : Instock_WPNotifier_Persistent_Notices
 * Description : Implements a Standardized messaging system that allows admin messages to be passed across page redirects.
 * */
if (!class_exists('Instock_WPNotifier_Persistent_Notices')) {

    class Instock_WPNotifier_Persistent_Notices {  // replace ClassName with the name of your class

        private static $_this;
        private $session_var = 'Instock_WPNotifier_Persistent_Notices';
        private $max_age;
        ...
        private function __construct() {
            ...
            add_action('admin_init', array($this, 'Instock_WPNotifie_admin_init'));
            add_action('admin_notices', array($this, 'Instock_WPNotifie_render_notices'));
            add_action('wp_ajax_get_persistent_notices', array($this, 'Instock_WPNotifie_ajax_get_notices'));
            add_action('shutdown', array($this, 'Instock_WPNotifie_save_notices'));
        }

        public function Instock_WPNotifie_admin_init() {
        ...
        public function Instock_WPNotifie_add_notice($notice = array()) {
            if (!is_array($notice)) {
                $notice = array(
                    'message' => $notice
                );
        ...
        public function Instock_WPNotifie_render_notices($location = 'default') {
            if (!$location) {
                $location = 'default';
            }
            ...
            $notices = $this->Instock_WPNotifie_retrieve_notices($location);
            $output_array = array();
            foreach ($notices as $notice) {
                ...
                if ($notice['in.html']) {
                    $output_array[] = wp_kses_post($notice['in.html']);
        ...
        public function Instock_WPNotifie_ajax_get_notices() {
            $location = 'default';
        ...
        public function Instock_WPNotifie_save_notices() {
            $notices = $this->Instock_WPNotifie_retrieve_notices('all');
            if (isset($notices)) {
        ...
        private function Instock_WPNotifie_retrieve_notices($location = 'default') {
            ...
            if (!apply_filters('wp_persistent_notices_prevent_retrieval', false)) {
                ...
                $raw_notices = apply_filters('wp_persistent_notices_replace_Instock_WPNotifie_retrieve_notices', false);
    ...
    Instock_WPNotifier_Persistent_Notices::Instance();

    if (!function_exists('instock_WPNotfier_add_persistent_notice')) {

        function instock_WPNotfier_add_persistent_notice($notice) {
            $cl = Instock_WPNotifier_Persistent_Notices::Instance();
            return $cl->Instock_WPNotifie_add_notice($notice);
        }

    }
    if (!function_exists('instock_WPNotfier_render_persistent_notices')) {

        function instock_WPNotfier_render_persistent_notices($location = 'default') {
            $cl = Instock_WPNotifier_Persistent_Notices::Instance();
            return $cl->Instock_WPNotifie_render_notices($location);
        }
